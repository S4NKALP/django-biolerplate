...
class AppManager:
    """Manages Django app creation and settings updates."""
    ...
    def create_app(self) -> bool:
        """Create a new Django app and add it to INSTALLED_APPS."""
        ...
        UIFormatter.print_success(
            f"Django app '{self.app_name}' created and configured successfully!"
        )
        return True
    ...
    def _create_django_app(self) -> bool:
        """Create the Django app using django-admin startapp."""
    ...
    def _add_to_installed_apps(self) -> bool:
        """Add the new app to INSTALLED_APPS in base.py settings."""
        ...
        if "USER_DEFINED_APPS" not in content:
            UIFormatter.print_error(
                "Could not find USER_DEFINED_APPS section in base.py"
            )
            return False
        ...
        UIFormatter.print_success(
            f"Added '{self.app_name}' to USER_DEFINED_APPS in base.py"
        )
        return True

    def _find_settings_path(self) -> Optional[str]:
        """Find the Django settings directory according to project structure."""
        ...
        for item in os.listdir(self.current_dir):
            if (
                ...
                if os.path.exists(settings_path) and os.path.exists(
                    os.path.join(settings_path, "base.py")
                ):
