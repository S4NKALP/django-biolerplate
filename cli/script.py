...
def validate_project_name(name: str) -> Tuple[bool, str]:
    ...
    reserved_names = {
        'test', 'tests', 'django', 'admin', 'api', 'static', 'media', 
        'manage', 'settings', 'urls', 'wsgi', 'asgi', 'models', 'views'
    }
...
def confirm_setup(project_name: str, app_name: str) -> bool:
    ...
    try:
        response = console.input(f"[{UIColors.WARNING}]Proceed with setup? (y/N):[/{UIColors.WARNING}] ")
...
def main() -> None:
    ...
    try:
        ...
        UIFormatter.print_error(f"Unexpected error: {str(e)}")
        console.print(f"[{UIColors.MUTED}]Please check your environment and try again.[/{UIColors.MUTED}]")
        sys.exit(1)
